 ...
 *
 * netlars: choose backlight path itself
 *
 * ...................
 * ********************************************************************/
...
int set_backlight_light(struct light_device_t* dev, struct light_state_t const* state)
{
	...
	if (access("/sys/class/backlight/rk28_bl/brightness", EXIST) != -1) {
		err = write_int("/sys/class/backlight/rk28_bl/brightness", brightness);
	}
	else if (access("/sys/class/backlight/rk30_bl/brightness", EXIST) != -1) {
		err = write_int("/sys/class/backlight/rk30_bl/brightness", brightness);
	}
